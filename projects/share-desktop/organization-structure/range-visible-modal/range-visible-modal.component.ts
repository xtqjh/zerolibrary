...
@Component({
  ...
  selector: 'zc-range-visible-modal',
  templateUrl: './range-visible-modal.component.html',
  styleUrls: ['./range-visible-modal.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None
})
export class RangeVisibleModalComponent extends MalihuScrollbarComponent {

  scrollbar$: Array<string> = ['#scrollbarA', '#scrollbarB', '#scrollbarC', '#scrollbarD', '#scrollbarSelect'];
  ...
  set isVisible(_data: boolean) {
    ...
    if (_data) {
      this.selectItems = [];
      // this.getDataLoad();
      if (this.tabParam.organizational) { this.setDeparMemberTreeSelect(this.nodesDMT, this.tools.isClone(this.beChosen)); }
      if (this.tabParam.tag) { this.setRecruitTagSelect(this.nodesRTT, this.tools.isClone(this.beChosen)); }
      if (this.tabParam.post) { this.setCompanyPostSelect(this.nodesCPT, this.tools.isClone(this.beChosen)); }
      if (this.tabParam.departmentPost) { this.setDepartmentPostTreeSelect(this.nodesDPT, this.tools.isClone(this.beChosen)); }
      setTimeout(() => {
        this.setNcksTREE();
        // this.selectItems = [];
  ...
  // 部门人员树结构原数据
  nodesDMT: NzTreeNodeOptions[] = [];

  // 岗位原数据
  nodesCPT: NzTreeNodeOptions[] = [];

  // 标签原数据
  nodesRTT: NzTreeNodeOptions[] = [];

  // 编制树结构原数据
  nodesDPT: NzTreeNodeOptions[] = [];
